# -*- coding: utf-8 -*-
# aliasfuzz repro | status=alias_diff backend=inductor device=cuda seed=71000659
import torch

def fn(t0, t1, ta, k, lst, dct):
    idx1 = torch.tensor([1], device=t0.device)
    t0[idx1] = -4
    v2 = ta.clone()
    v3 = v2 + ta
    v3[1:2:2, :, :] = -5
    v4 = v3.reshape(v3.shape)
    v5 = torch.slice_scatter(ta, v3[2:3:2, :, :], dim=0, start=2, end=3, step=2)
    return (t1, v2, v5, v3, t1, lst, dct)

INPUTS = [('t0', (4, 3, 2)), ('t1', (2, 3, 4))]
