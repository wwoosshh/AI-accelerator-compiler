# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=71000415
import torch

def fn(t0, t1, ta, k, lst, dct):
    v1 = torch.minimum(t0, t1)
    v2 = v1.cumsum(2)
    v3 = t1.unfold(0, 1, 2)
    v4 = v2 + t0
    v5 = ta.reshape((1, 24))
    v2[:2:2, :, :] = v1[:2:2, :, :]
    v6 = v5 - -1
    idx7 = torch.tensor([0], device=v3.device)
    v3.index_fill_(0, idx7, 2)
    return (v4, ta, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (2, 3, 4))]
