# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=71000978
import torch

def fn(t0, t1, t2, ta, k, lst, dct):
    idx1 = torch.tensor([2, 0, 1], device=t1.device)
    t1.scatter_add_(1, (t0.abs() % 3).long(), t0)
    v2 = t2.reshape((2, 2, 6))
    t1.copy_(t0)
    v3 = ta[:-2:3, :1:3, 1:2]
    idx4 = torch.tensor([0, 2], device=ta.device)
    ta.index_fill_(1, idx4, k)
    v5 = v3.unsqueeze(0).expand(3, 1, 1, 1)
    return (t0, t1, t0, lst, dct)

INPUTS = [('t0', (4, 3, 2)), ('t1', (4, 3, 2)), ('t2', (24,))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'f64'
DYNAMIC = True
DEVICE = 'cuda'
