# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=71001143
import torch

def fn(t0, t1, k, lst, dct):
    v1 = t1.unsqueeze(2)
    v2 = v1.unfold(1, 2, 2)
    v3 = v1.unsqueeze(0).expand(2, 4, 3, 1, 2)
    v4 = v2.select(0, 3)
    v5 = v1.unsqueeze(4)
    v6 = v3.detach()
    v7 = torch.slice_scatter(v6, v3[0:2:2, :, :, :, :], dim=0, start=0, end=2, step=2)
    idx8 = torch.tensor([0, 1], device=v2.device)
    v2.index_fill_(4, idx8, k)
    return (v1, v4, v5, t0, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (4, 3, 2))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
