# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=71000788
import torch

def fn(t0, t1, t2, k, lst, dct):
    dct['key1'] = t0
    v2 = t0.view(torch.int32)
    t1[t1 > 0] = -3
    v3 = t0.detach()
    v4 = torch.diagonal_scatter(v3, t0.diagonal())
    idx5 = torch.tensor([1, 0], device=t1.device)
    t1.index_fill_(0, idx5, -2)
    v3.add_(-5)
    t0.fill_(4)
    return (v4, t0, t1, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (2, 12)), ('t2', (4, 3, 2))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
