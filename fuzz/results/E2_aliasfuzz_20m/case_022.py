# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=71001431
import torch

def fn(t0, t1, t2, ta, k, lst, dct):
    v1 = t2.unfold(1, 2, 3)
    idx2 = torch.tensor([23], device=t1.device)
    t1[idx2] += 3
    v3 = t0.view(torch.int32)
    v4 = t0.unfold(0, 18, 4)
    lst.append(v4.numel())
    v5 = v1.reshape(v1.shape)
    v6 = t2.view(torch.int32)
    idx7 = torch.tensor([0], device=v5.device)
    v5.index_fill_(1, idx7, 3)
    return (ta, v4, t0, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (24,)), ('t2', (2, 3, 4))]
ALIASED = ('ta', 't0')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
