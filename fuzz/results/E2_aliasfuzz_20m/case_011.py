# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=71000674
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t2.reshape((6, 4))
    v2 = v1.permute(1, 0)
    v3 = t2.unsqueeze(0).expand(2, 2, 12)
    v4 = v1.unfold(0, 5, 6)
    idx5 = torch.tensor([1, 0, 3], device=t1.device)
    t1.index_add_(0, idx5, v2.narrow(0, 0, 3))
    v4.sub_(-1)
    v6 = t1.detach()
    t2[t2 > 1] = 4
    return (v1, v4, v1, t2, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (4, 6)), ('t2', (2, 12))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = True
DEVICE = 'cuda'
