# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41002377
import torch
...
def fn(t0, t1, t2, ta, k, lst, dct):
    ...
    v2 = t2[2:4:2, :, :-1:2]
    v1.abs_()
    v3 = torch.slice_scatter(ta, t0[2:3:2, :], dim=0, start=2, end=3, step=2)
    v4 = ta.unsqueeze(0).expand(3, 4, 6)
    return (v4, v1, t2, lst, dct)

INPUTS = [('t0', (4, 6)), ('t1', (2, 12)), ('t2', (4, 3, 2))]
ALIASED = ('ta', 't0')
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cpu'
