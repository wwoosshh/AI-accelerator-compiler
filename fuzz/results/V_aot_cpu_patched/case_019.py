# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41002096
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t0.unfold(2, 3, 4)
    v2 = v1.select(2, 0)
    idx3 = torch.tensor([2], device=v1.device)
    v1.index_fill_(1, idx3, k)
    t2[1:3, 7:8] = -3
    v4 = v2.unsqueeze(0).expand(3, 2, 3, 3)
    v5 = v4.transpose(3, 2)
    t0.masked_fill_(t0 > 0, -4)
    v6 = torch.slice_scatter(t2, t1[1:2:2, :], dim=0, start=1, end=2, step=2)
    v7 = v4[:, 1::2, :1, 0:2:2]
    return (v6, v7, v6, t1, t2, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (3, 8)), ('t2', (3, 8))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
