# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41002655
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t1[:]
    t0.add_(k)
    idx2 = torch.tensor([0, 3], device=t1.device)
    t1[idx2] = 1
    v3 = t2.unfold(2, 1, 4)
    dct['n'] = dct.get('n', 0) + 1
    idx4 = torch.tensor([1, 0], device=v3.device)
    v3.index_fill_(0, idx4, -1)
    return (t2, v1, t2, lst, dct)

INPUTS = [('t0', (3, 8)), ('t1', (4, 6)), ('t2', (2, 3, 4))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cpu'
