# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41003018
import torch

def fn(t0, t1, t2, ta, k, lst, dct):
    v1 = ta.narrow(1, 0, 2)
    idx2 = torch.tensor([0, 1], device=t2.device)
    t2.index_fill_(0, idx2, k)
    v3 = t1.unfold(0, 1, 4)
    t2.mul_(-1)
    v3.add_(5)
    v4 = t0.reshape((4, 3, 2))
    return (v3, t0, t2, lst, dct)

INPUTS = [('t0', (4, 6)), ('t1', (4, 6)), ('t2', (2, 2, 6))]
