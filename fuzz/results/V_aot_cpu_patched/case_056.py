# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41006375
import torch

def fn(t0, t1, ta, k, lst, dct):
    v1 = ta.unfold(0, 11, 19)
    t1[:-1, :, :] = t1[1:, :, :].clone()
    v1[:, :] = 3
    t1[t1 > 0] = 2
    idx2 = torch.tensor([2, 7], device=v1.device)
    v1.index_fill_(1, idx2, 5)
    dct['key3'] = ta
    v4 = t1.view((2, 2, 6))
    idx5 = torch.tensor([11, 3], device=ta.device)
    ta.index_fill_(0, idx5, 1)
    return (v4, t1, ta, t1, v4, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (2, 3, 4))]
ALIASED = ('ta', 't0.view(-1)')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
BACKEND = 'aot_eager'
