# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41006308
import torch
...
def fn(t0, t1, ta, k, lst, dct):
    ...
    v1 = t0.unfold(1, 1, 1)
    v2 = ta.unfold(2, 4, 4)
    v2.masked_fill_(v2 > 2, k)
    idx3 = torch.tensor([0], device=ta.device)
    ta.index_fill_(1, idx3, -2)
    return (ta, ta, ta, ta, t0, lst, dct)

INPUTS = [('t0', (2, 2, 6)), ('t1', (4, 6))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
