# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41003675
import torch
...
def fn(t0, t1, k, lst, dct):
    ...
    v2 = v1.unfold(2, 2, 3)
    v3 = v1 + 1
    idx4 = torch.tensor([0, 1, 2], device=t0.device)
    t0.index_fill_(1, idx4, -3)
    v5 = v3.permute(3, 1, 0, 2)
    v2.add_(v2.flip(2))
    idx6 = torch.tensor([0], device=v1.device)
    v1.scatter_add_(2, (v3.abs() % 3).long(), v3)
    v7 = v2[:1:3, :, 0, 2:3, -2]
    return (v1, v1, t0, v2, t1, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (4, 3, 2))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
