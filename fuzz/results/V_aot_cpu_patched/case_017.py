# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41001787
import torch

def fn(t0, t1, k, lst, dct):
    v1 = t0[2:4:3, 1:2, :]
    idx2 = torch.tensor([1, 3], device=t0.device)
    t0.index_fill_(0, idx2, -2)
    v3 = t1.unfold(0, 9, 18)
    idx4 = torch.tensor([0], device=v1.device)
    v1.index_fill_(1, idx4, k)
    v3.fill_(-3)
    v5 = t0[:, 2:, 1:2:2]
    v1.add_(-2)
    return (v1, t1, v3, t1, lst, dct)

INPUTS = [('t0', (4, 3, 2)), ('t1', (24,))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = True
DEVICE = 'cpu'
