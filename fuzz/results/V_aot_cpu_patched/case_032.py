# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41003767
import torch

def fn(t0, t1, t2, k, lst, dct):
    idx1 = torch.tensor([1], device=t1.device)
    t1.index_copy_(2, idx1, t2.narrow(2, 0, 1))
    t1.sub_(t2)
    v2 = t0.reshape(t0.shape)
    v3 = t1.cumsum(1)
    v4 = t2.unfold(1, 2, 3)
    v5 = t2.select(2, 2)
    lst.append(v2.numel())
    v4[v4 > -2] = 5
    return (v5, t2, t1, v5, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (2, 3, 4)), ('t2', (2, 3, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
