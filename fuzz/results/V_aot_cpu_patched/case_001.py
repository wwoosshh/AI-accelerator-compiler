# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41000136
import torch

def fn(t0, t1, t2, k, lst, dct):
    t0.copy_(t1)
    v1 = torch.ops.aten.alias(t0)
    v2 = t2.unfold(0, 1, 3)
    v3 = t1.transpose(0, 1)
    v3.fill_(-3)
    v2[1:, :, :] = v2[:-1, :, :].clone()
    ...
    v1.transpose_(0, 1)
    v5 = t1[:]
    return (v3, t0, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (2, 12)), ('t2', (6, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
