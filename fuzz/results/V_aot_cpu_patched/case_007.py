# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41001150
import torch

def fn(t0, t1, t2, ta, k, lst, dct):
    v1 = t2.unsqueeze(0)
    v2 = t2.clamp(-4, 3)
    v3 = t2.unfold(0, 21, 22)
    v2.unsqueeze_(1)
    idx4 = torch.tensor([17, 7], device=v3.device)
    v3.index_fill_(1, idx4, k)
    v5 = ta.reshape((20,))
    t2[-11] = k
    v6 = v2[13:16, :]
    v6.add_(v6)
    return (v6, v3, t0, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (2, 2, 6)), ('t2', (24,))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
