# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41004864
import torch

def fn(t0, t1, t2, k, lst, dct):
    idx1 = torch.tensor([0, 1], device=t0.device)
    t0.index_fill_(1, idx1, k)
    v2 = t1.unfold(1, 1, 3)
    t2[t2 > -2] = k
    v2.clamp_(-4, 3)
    t1[0::2, :, :] = k
    v2.transpose_(3, 1)
    v3 = v2[:]
    return (v3, v3, v3, t2, t1, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (4, 3, 2)), ('t2', (2, 12))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
