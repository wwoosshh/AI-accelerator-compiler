# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41001525
import torch

def fn(t0, t1, k, lst, dct):
    t1.add_(t1)
    v1 = t0.unfold(1, 1, 2)
    v2 = torch.roll(t0, 2, 1)
    v3 = t1[0, :]
    idx4 = torch.tensor([0], device=v1.device)
    v1[idx4] = 2
    v2.masked_fill_(v2 > 0, k)
    v5 = t1.neg()
    dct['key6'] = v5
    return (v2, v5, t0, v1, t0, lst, dct)

INPUTS = [('t0', (2, 2, 6)), ('t1', (2, 12))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cpu'
