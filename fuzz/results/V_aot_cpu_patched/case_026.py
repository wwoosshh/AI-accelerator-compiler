# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41002950
import torch

def fn(t0, t1, ta, k, lst, dct):
    v1 = t1.reshape((6, 4))
    v2 = ta.unfold(0, 12, 21)
    v1.fill_(-5)
    v3 = t1.transpose(0, 1)
    v2[:, 10:12] += k
    idx4 = torch.tensor([14, 0, 2], device=ta.device)
    ta[idx4] = k
    v5 = v2.flip(0)
    v5.masked_fill_(v5 > -2, -1)
    return (t1, v1, v3, t0, lst, dct)
