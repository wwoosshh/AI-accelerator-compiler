# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41003972
import torch

def fn(t0, t1, t2, k, lst, dct):
    t2.masked_fill_(t2 > 2, -2)
    t0[14:18] = t1[14:18]
    v1 = t2.t()
    t1.add_(t0)
    v2 = t1.unfold(0, 18, 24)
    v1[0] = 0
    v1.sub_(-1)
    idx3 = torch.tensor([0], device=v2.device)
    v2[idx3] += -5
    return (v2, t1, t0, t0, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (24,)), ('t2', (3, 8))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cpu'
