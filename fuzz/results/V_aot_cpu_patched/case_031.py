# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=41003702
import torch

def fn(t0, t1, ta, k, lst, dct):
    ta[1:2, :] = 5
    v1 = ta.unfold(1, 3, 4)
    idx2 = torch.tensor([0, 4], device=ta.device)
    ta.index_fill_(1, idx2, -4)
    idx3 = torch.tensor([0], device=v1.device)
    v1[idx3] += k
    v4 = t0.view(torch.int32)
    return (v1, v4, t1, ta, t1, v1, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (4, 6))]
ALIASED = ('ta', 't0')
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cpu'
