# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=51001397
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t2.abs()
    idx2 = torch.tensor([0, 1], device=v1.device)
    v1.index_add_(0, idx2, t2.narrow(0, 0, 2))
    v3 = t1.unfold(2, 4, 5)
    v4 = torch.slice_scatter(v1, t2[:, :, 1:2:3], dim=2, start=1, end=2, step=3)
    lst.append(t2.numel())
    v5 = v3[:, 1:2, :, :]
    v6 = v4.neg()
    v7 = v3.transpose(0, 3)
    idx8 = torch.tensor([0, 1], device=v3.device)
    v3.index_fill_(0, idx8, -1)
    return (v7, v5, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (2, 2, 6)), ('t2', (4, 3, 2))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
