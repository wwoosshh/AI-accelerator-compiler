# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=51001631
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t2.view(torch.int32)
    v2 = t2.unfold(1, 1, 8)
    idx3 = torch.tensor([0, 2, 1], device=t1.device)
    t1.index_fill_(1, idx3, -3)
    v4 = t2[:, 9:10:2]
    v5 = v2.squeeze(2)
    idx6 = torch.tensor([1], device=v5.device)
    v5[idx6] = 0
    return (t0, v2, v4, t0, lst, dct)

INPUTS = [('t0', (4, 3, 2)), ('t1', (2, 3, 4)), ('t2', (2, 12))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
