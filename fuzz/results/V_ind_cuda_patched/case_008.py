# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=51000729
import torch

def fn(t0, t1, ta, k, lst, dct):
    idx1 = torch.tensor([2], device=t0.device)
    t0.index_copy_(0, idx1, t1.narrow(0, 0, 1))
    t0[t1 > -1] = k
    v2 = t0.cumsum(1)
    v3 = ta.contiguous()
    v4 = v3.reshape((6, 4))
    ta.unsqueeze_(1)
    return (v2, v2, v3, t0, v2, lst, dct)

INPUTS = [('t0', (3, 8)), ('t1', (3, 8))]
