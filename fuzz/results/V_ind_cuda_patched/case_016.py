# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=51001350
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = torch.maximum(t2, t1)
    dct['n'] = dct.get('n', 0) + 1
    t0[22:23] = -5
    v2 = t2.diagonal()
    v3 = v2.unfold(0, 1, 3)
    ...
    v2.index_fill_(0, idx5, k)
    v3[:1:3, :] = -1
    t1.sub_(t2.narrow(0, 0, 1))
    return (v3, v3, v3, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (4, 6)), ('t2', (4, 6))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
