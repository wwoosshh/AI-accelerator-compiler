# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=51000314
import torch
...
def fn(t0, t1, t2, k, lst, dct):
    ...
    t1.mul_(2)
    t1[1:, :] = t1[:-1, :].clone()
    v1 = t2.unfold(0, 12, 20)
    v2 = t2.view(torch.int32)
    v3 = v1.select(1, 8)
    v1.add_(v1.flip(0))
    v2.masked_fill_(v2 > -2, k)
    return (v3, t0, v3, v3, t0, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (4, 6)), ('t2', (24,))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
