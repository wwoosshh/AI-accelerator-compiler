# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=51000761
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t1.unfold(1, 1, 2)
    idx2 = torch.tensor([2, 1], device=t2.device)
    t2.index_add_(1, idx2, t1.narrow(1, 0, 2))
    v3 = t0.cumsum(0)
    v4 = t2.view_as(t2)
    t1[1] = 3
    v1.sub_(-5)
    return (t2, v3, t2, t2, lst, dct)

INPUTS = [('t0', (2, 2, 6)), ('t1', (4, 3, 2)), ('t2', (4, 3, 2))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = True
DEVICE = 'cuda'
