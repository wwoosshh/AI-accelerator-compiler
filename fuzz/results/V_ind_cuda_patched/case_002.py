# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=51000312
import torch

def fn(t0, t1, k, lst, dct):
    v1 = t1.view((4, 6))
    t1[:-1] = t1[1:].clone()
    v2 = t1.unfold(0, 10, 17)
    v3 = torch.ops.aten.alias(v1)
    v4 = v3.t()
    idx5 = torch.tensor([0], device=v2.device)
    v2.index_fill_(0, idx5, k)
    return (v4, v3, t0, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (24,))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
