# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=51000869
import torch

def fn(t0, t1, t2, k, lst, dct):
    t2.unsqueeze_(0)
    v1 = t0.unfold(1, 8, 8)
    idx2 = torch.tensor([1, 5, 11], device=t0.device)
    t0.index_fill_(1, idx2, -4)
    v1.clamp_(-1, 4)
    v3 = t0.reshape(t0.shape)
    v4 = t2[...]
    v5 = t0.view(torch.int32)
    v6 = v3[1:, :]
    return (v5, t1, t0, v4, t0, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (24,)), ('t2', (24,))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
