# -*- coding: utf-8 -*-
# aliasfuzz repro | status=alias_diff backend=inductor device=cuda seed=1000092
import torch

def fn(t0, t1, t2, ta, k, lst, dct):
    v1 = ta[:, :]
    idx2 = torch.tensor([5], device=ta.device)
    ta.index_fill_(1, idx2, -1)
    v3 = v1 + 0
    return (t2, v3, v3, t2, t2, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (6, 4)), ('t2', (4, 3, 2))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
