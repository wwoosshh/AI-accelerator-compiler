# -*- coding: utf-8 -*-
# aliasfuzz repro | status=alias_diff backend=inductor device=cuda seed=1000120
import torch
...
def fn(t0, t1, t2, ta, k, lst, dct):
    ...
    v2 = t2 + 0
    v3 = v2.unfold(1, 3, 2)
    v4 = torch.diagonal_scatter(t1, t0.diagonal())
    return (v2, t0, v4, t0, lst, dct)

INPUTS = [('t0', (3, 8)), ('t1', (3, 8)), ('t2', (6, 4))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
