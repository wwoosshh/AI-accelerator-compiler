# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=1000098
import torch
...
def fn(t0, t1, t2, ta, k, lst, dct):
    ...
    v3 = t0 + -4
    v1.add_(v1)
    idx4 = torch.tensor([1], device=v2.device)
    v2[idx4] += k
    ta[-21] = k
    ta.abs_()
    idx5 = torch.tensor([2, 19, 14], device=ta.device)
    ta.index_fill_(0, idx5, 4)
    return (t0, t2, lst, dct)
