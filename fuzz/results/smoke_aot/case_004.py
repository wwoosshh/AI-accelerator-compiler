# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=1000195
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t2.view_as(t2)
    v1.sub_(t2.narrow(2, 0, 1))
    v2 = t2.unsqueeze(2)
    lst.append(t0)
    v3 = t0.cumsum(0)
    v4 = torch.select_scatter(v1, t2.select(0, 0), 0, 0)
    idx5 = torch.tensor([2, 1, 0], device=t2.device)
    t2.index_add_(1, idx5, v4.narrow(1, 0, 3))
    v3[1] = -1
    return (v2, t2, t0, t1, v2, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (6, 4)), ('t2', (2, 3, 4))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cpu'
