# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=72002863
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = torch.select_scatter(t0, t2.select(1, 0), 1, 0)
    idx2 = torch.tensor([2, 1, 3], device=t1.device)
    t1[idx2] = 2
    v3 = t2.unfold(2, 5, 6)
    v3.add_(v3.flip(3))
    v4 = t1.view(torch.int32)
    v5 = v1.reshape(v1.shape)
    return (v5, t0, t1, t0, lst, dct)

INPUTS = [('t0', (2, 2, 6)), ('t1', (4, 3, 2)), ('t2', (2, 2, 6))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
