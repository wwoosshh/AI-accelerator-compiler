# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=72001329
import torch

def fn(t0, t1, ta, k, lst, dct):
    idx1 = torch.tensor([2, 0], device=t1.device)
    t1[idx1] += -2
    v2 = torch.slice_scatter(t0, ta[0:1:2, :], dim=0, start=0, end=1, step=2)
    v3 = t1 + -1
    v4 = t0.view(torch.int32)
    v5 = v2.view((24,))
    v6 = v5 * -1
    v4[v4 > 2] = k
    v5[-18] = -1
    v7 = torch.minimum(v5, v6)
    return (v6, v4, v3, t1, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (4, 3, 2))]
ALIASED = ('ta', 't0')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
