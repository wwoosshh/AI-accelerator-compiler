# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=72002591
import torch
...
def fn(t0, t1, k, lst, dct):
    ...
    v1 = t0.unfold(1, 4, 10)
    t1[:-1] = t1[1:].clone()
    v1.masked_fill_(v1 > 2, k)
    t0.clamp_(-4, 4)
    t1.add_(t1.flip(0))
    v2 = t1.unfold(0, 7, 7)
    v2.fill_(-3)
    idx3 = torch.tensor([15, 6], device=t1.device)
    t1.index_fill_(0, idx3, 0)
    return (v2, v1, t0, t1, t0, v2, lst, dct)
