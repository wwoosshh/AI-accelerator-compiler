# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=72001747
import torch

def fn(t0, t1, t2, k, lst, dct):
    t0.masked_fill_(t0 > -1, 2)
    v1 = t0[:, :, :]
    v2 = v1.to(v1.dtype)
    idx3 = torch.tensor([1], device=t0.device)
    t0.index_fill_(0, idx3, 4)
    t0[t0 > 2] = 0
    v2.sub_(k)
    v4 = v2.unsqueeze(2)
    v5 = t2.narrow(1, 8, 4)
    idx6 = torch.tensor([0, 1], device=v2.device)
    v2.index_fill_(1, idx6, -5)
    return (v4, t1, v4, t1, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (2, 2, 6)), ('t2', (2, 12))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cuda'
