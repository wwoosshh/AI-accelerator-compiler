# -*- coding: utf-8 -*-
# aliasfuzz repro | status=alias_diff backend=inductor device=cuda seed=72000598
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t1.transpose(1, 0)
    v2 = v1.flip(1)
    v3 = v2.unsqueeze(0).expand(3, 2, 2, 6)
    v2.copy_(v1)
    t1.unsqueeze_(1)
    idx4 = torch.tensor([2], device=t0.device)
    t0.index_fill_(2, idx4, 5)
    v5 = v3[2, :, :, :]
    v6 = v5.flatten()
    return (v6, t0, v3, v6, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (2, 2, 6)), ('t2', (24,))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
