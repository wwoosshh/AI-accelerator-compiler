# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=72002116
import torch
...
def fn(t0, t1, k, lst, dct):
    ...
    v1.clamp_(-3, 1)
    v2 = t0.clone()
    v3 = torch.cat([v2, t0], 2)
    idx4 = torch.tensor([3], device=v3.device)
    v3.index_fill_(2, idx4, 2)
    v5 = t1.permute(1, 0)
    lst.append(v1.numel())
    v6 = v1.narrow(2, 1, 1)
    return (t0, v5, t1, t0, lst, dct)

INPUTS = [('t0', (4, 3, 2)), ('t1', (6, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
