# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=72001526
import torch

def fn(t0, t1, ta, k, lst, dct):
    v1 = torch.ops.aten.alias(ta)
    idx2 = torch.tensor([3], device=t0.device)
    t0.index_fill_(0, idx2, k)
    v3 = t1.unsqueeze(0)
    ta[0] = 3
    v4 = ta[2:-2:2, :]
    return (v4, v4, t0, v4, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (2, 2, 6))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
