# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=72002342
import torch

def fn(t0, t1, ta, k, lst, dct):
    v1 = ta[0, 1:3]
    v1.add_(v1)
    v2 = torch.select_scatter(t0, t1.select(1, 0), 1, 0)
    idx3 = torch.tensor([2, 1, 3], device=t0.device)
    t0.index_fill_(1, idx3, k)
    return (ta, v1, ta, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (6, 4))]
