# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11000395
import torch

def fn(t0, t1, k, lst, dct):
    idx1 = torch.tensor([1, 2, 3], device=t0.device)
    t0[idx1] += -1
    t1[2:4:2, :] = t0[2:4:2, :]
    v2 = t1.unfold(1, 3, 5)
    v3 = v2[2:4:3, :, -1]
    v4 = v3.transpose(1, 0)
    idx5 = torch.tensor([0, 2, 1], device=v2.device)
    v2.index_fill_(2, idx5, k)
    return (v2, v4, t1, lst, dct)

INPUTS = [('t0', (4, 6)), ('t1', (4, 6))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cpu'
