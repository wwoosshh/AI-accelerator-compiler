# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11007222
import torch
...
def fn(t0, t1, k, lst, dct):
    ...
    t1.sub_(0)
    v3 = t0.unfold(1, 1, 2)
    idx4 = torch.tensor([5], device=v3.device)
    v3.index_fill_(0, idx4, k)
    idx5 = torch.tensor([2, 0, 1], device=t1.device)
    t1.index_fill_(1, idx5, k)
    v6 = v3.view_as(v3)
    idx7 = torch.tensor([5, 3, 0], device=t0.device)
    t0.index_fill_(0, idx7, -2)
    return (v6, v6, v1, v6, v6, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (2, 3, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
