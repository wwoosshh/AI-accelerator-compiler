# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11004795
import torch
...
def fn(t0, t1, ta, k, lst, dct):
    ...
    v1[:, :, :] += 3
    v2 = t0[:, 3:4:3]
    v3 = v1.reshape(v1.shape)
    idx4 = torch.tensor([1], device=t0.device)
    t0.index_fill_(0, idx4, 3)
    idx5 = torch.tensor([0, 2], device=v3.device)
    v3.index_fill_(2, idx5, 0)
    return (ta, v3, t0, lst, dct)

INPUTS = [('t0', (3, 8)), ('t1', (2, 2, 6))]
ALIASED = ('ta', 't0.transpose(0, 1)')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
BACKEND = 'aot_eager'
