# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11007081
import torch

def fn(t0, t1, k, lst, dct):
    t0.masked_fill_(t1 > 2, 5)
    t0.copy_(t1)
    v1 = t0.unfold(0, 2, 3)
    v2 = torch.ops.aten.alias(t0)
    idx3 = torch.tensor([1], device=v1.device)
    v1.index_fill_(2, idx3, 4)
    v4 = t0.select(1, 0)
    v4.add_(5)
    v5 = v2.unfold(1, 2, 1)
    return (v1, v1, v1, v5, v1, lst, dct)

INPUTS = [('t0', (4, 3, 2)), ('t1', (4, 3, 2))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
