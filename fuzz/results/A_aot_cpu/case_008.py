# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11001294
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t2.view(torch.int32)
    idx2 = torch.tensor([1, 0], device=t0.device)
    t0.index_fill_(2, idx2, 3)
    v3 = t1.unfold(0, 1, 2)
    v3.add_(k)
    v4 = v3.neg()
    v5 = v4.transpose(1, 2)
    v6 = v5.reshape((12,))
    idx7 = torch.tensor([11, 6], device=v6.device)
    v6.index_fill_(0, idx7, k)
    return (t1, t2, t2, lst, dct)

INPUTS = [('t0', (4, 3, 2)), ('t1', (2, 3, 4)), ('t2', (2, 3, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cpu'
