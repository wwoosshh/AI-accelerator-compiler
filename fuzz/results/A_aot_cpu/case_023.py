# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11002532
import torch

def fn(t0, t1, ta, k, lst, dct):
    v1 = t1.unfold(1, 7, 12)
    v2 = torch.slice_scatter(ta, t1[:, 1:4:1], dim=1, start=1, end=4, step=1)
    idx3 = torch.tensor([6, 5], device=ta.device)
    ta.index_fill_(1, idx3, 4)
    idx4 = torch.tensor([1, 0], device=v1.device)
    v1[idx4] += -1
    v5 = v1.reshape((14,))
    v6 = v2.select(0, 0)
    v7 = v5.clamp(0, 4)
    return (v2, t1, t1, lst, dct)
