# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11003248
import torch

def fn(t0, t1, ta, k, lst, dct):
    idx1 = torch.tensor([7, 2], device=ta.device)
    ta.scatter_add_(0, (t1.abs() % 24).long(), t1)
    idx2 = torch.tensor([21, 22], device=ta.device)
    ta[idx2] = k
    lst.append(t1.numel())
    v3 = ta.unfold(0, 6, 14)
    v4 = v3.unsqueeze(2)
