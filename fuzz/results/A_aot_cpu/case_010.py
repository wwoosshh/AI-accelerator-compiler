# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11001436
import torch
...
def fn(t0, t1, t2, k, lst, dct):
    ...
    t1.fill_(4)
    v2 = v1.unfold(1, 1, 2)
    t0.masked_fill_(t0 > 1, -5)
    v2.sub_(2)
    t0.neg_()
    v3 = t2.unsqueeze(0)
    idx4 = torch.tensor([0, 2, 1], device=v3.device)
    v3.index_fill_(1, idx4, k)
    v5 = t2.unsqueeze(2)
    return (t1, t2, v3, t1, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (4, 6)), ('t2', (3, 8))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = True
DEVICE = 'cpu'
