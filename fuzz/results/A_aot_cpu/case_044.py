# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11004758
import torch

def fn(t0, t1, t2, k, lst, dct):
    idx1 = torch.tensor([2, 1, 3], device=t2.device)
    t2.index_fill_(1, idx1, k)
    t2.transpose_(1, 0)
    v2 = t2.flatten()
    v3 = t1.unfold(0, 2, 3)
    v4 = t2.sum(1, keepdim=True)
    v3.add_(-2)
    return (v4, t1, t1, lst, dct)

INPUTS = [('t0', (4, 3, 2)), ('t1', (3, 8)), ('t2', (6, 4))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cpu'
