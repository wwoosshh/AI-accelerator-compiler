# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11002262
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = t1.unsqueeze(3)
    t2.sub_(t0)
    idx2 = torch.tensor([1], device=t0.device)
    t0.scatter_add_(0, (t2.abs() % 2).long(), t2)
    v3 = t2.unsqueeze(0).expand(3, 2, 3, 4)
    v4 = t1[:, 1:, 0]
    v5 = t2.permute(2, 0, 1)
    v6 = t2.unfold(2, 1, 2)
    v6.add_(-5)
    return (v1, v3, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (4, 3, 2)), ('t2', (2, 3, 4))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = True
DEVICE = 'cpu'
