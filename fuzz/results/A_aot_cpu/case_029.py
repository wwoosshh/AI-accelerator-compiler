# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11003007
import torch

def fn(t0, t1, k, lst, dct):
    idx1 = torch.tensor([2, 1, 0], device=t0.device)
    t0.index_fill_(0, idx1, k)
    v2 = t0.unfold(1, 7, 8)
    v2[1:, :, :] = v2[:-1, :, :].clone()
    v3 = v2.flatten()
    v4 = v3[:]
    return (v4, v4, v4, lst, dct)

INPUTS = [('t0', (3, 8)), ('t1', (4, 3, 2))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
