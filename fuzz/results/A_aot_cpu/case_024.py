# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11002561
import torch

def fn(t0, t1, t2, ta, k, lst, dct):
    lst.append(t1.shape[2] + k)
    v1 = t1.unfold(0, 2, 4)
    t1[t1 > -2] = -3
    idx2 = torch.tensor([0, 1], device=v1.device)
    v1.index_fill_(3, idx2, 2)
    dct['key3'] = t1
    v4 = v1 - -1
    t2.t_()
    return (v4, t2, lst, dct)

INPUTS = [('t0', (2, 2, 6)), ('t1', (4, 3, 2)), ('t2', (4, 6))]
ALIASED = ('ta', 't0')
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cpu'
