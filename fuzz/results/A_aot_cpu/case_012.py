# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11001508
import torch
...
def fn(t0, t1, t2, k, lst, dct):
    ...
    v1[v1 > 0] = 4
    t0[:, 0:3:2, 3:] += 0
    idx3 = torch.tensor([1], device=v2.device)
    v2.index_fill_(0, idx3, 5)
    idx4 = torch.tensor([1, 0, 3], device=t2.device)
    t2.index_fill_(2, idx4, 0)
    t2[:-1, :, :] = t2[1:, :, :].clone()
    return (t2, t2, v1, t0, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (4, 6)), ('t2', (2, 2, 6))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cpu'
