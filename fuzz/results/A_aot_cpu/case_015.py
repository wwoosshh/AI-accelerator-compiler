# -*- coding: utf-8 -*-
# aliasfuzz repro | status=alias_diff backend=aot_eager device=cpu seed=11001917
import torch

def fn(t0, t1, t2, ta, k, lst, dct):
    idx1 = torch.tensor([1, 0, 3], device=t2.device)
    t2.index_fill_(1, idx1, 1)
    v2 = t0.reshape((24,))
    v3 = ta.narrow(0, 0, 2)
    v4 = t0.transpose(0, 1)
    ta[ta > 1] = 3
    ...
    v2[-1] = 1
    v2[v2 > 0] = 0
    v5 = v2.view(torch.int32)
    return (v3, ta, t1, lst, dct)

INPUTS = [('t0', (4, 6)), ('t1', (2, 2, 6)), ('t2', (6, 4))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
