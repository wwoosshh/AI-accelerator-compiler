# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11000832
import torch
...
def fn(t0, t1, ta, k, lst, dct):
    ...
    v2 = ta.detach()
    dct['key3'] = v1
    v4 = torch.select_scatter(t0, v2.select(1, 0), 1, 0)
    idx5 = torch.tensor([1], device=v1.device)
    v1.scatter_add_(1, (v4.abs() % 2).long(), v4)
    return (v4, v1, t0, v2, lst, dct)

INPUTS = [('t0', (2, 2, 6)), ('t1', (6, 4))]
