# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11000149
import torch
...
def fn(t0, t1, t2, k, lst, dct):
    ...
    v2 = t1.unfold(0, 1, 2)
    v3 = v1[:]
    v4 = torch.slice_scatter(v1, v3[0:1:1], dim=0, start=0, end=1, step=1)
    idx5 = torch.tensor([0], device=v2.device)
    v2.index_fill_(3, idx5, -4)
    v6 = t0.flip(0)
    return (v1, t0, v6, t1, t2, lst, dct)

INPUTS = [('t0', (4, 6)), ('t1', (2, 2, 6)), ('t2', (24,))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cpu'
