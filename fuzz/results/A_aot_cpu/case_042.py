# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11004291
import torch

def fn(t0, t1, k, lst, dct):
    idx1 = torch.tensor([2], device=t1.device)
    t1.index_fill_(1, idx1, k)
    v2 = t0.unfold(0, 5, 5)
    t1.sub_(1)
    v3 = v2[-1, -1, :]
    v3.add_(v3.flip(0))
    idx4 = torch.tensor([0], device=v2.device)
    v2.index_fill_(0, idx4, 2)
    return (t1, t1, t1, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (4, 6))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cpu'
