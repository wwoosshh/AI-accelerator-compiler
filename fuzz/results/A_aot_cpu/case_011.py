# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=aot_eager device=cpu seed=11001447
import torch

def fn(t0, t1, ta, k, lst, dct):
    v1 = t0.unfold(0, 14, 16)
    t1.add_(t1.flip(0))
    v1.add_(5)
    idx2 = torch.tensor([9], device=ta.device)
    ta.index_fill_(0, idx2, 1)
    v3 = v1.reshape((14,))
    v1.masked_fill_(v1 > -1, k)
    v4 = t1.narrow(0, 1, 1)
    return (v4, t1, v4, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (2, 2, 6))]
ALIASED = ('ta', 't0')
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cpu'
