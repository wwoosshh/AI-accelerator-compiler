# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21001805
import torch

def fn(t0, t1, k, lst, dct):
    v1 = torch.slice_scatter(t0, t1[2:3:1, :, :], dim=0, start=2, end=3, step=1)
    v2 = v1.cumsum(1)
    idx3 = torch.tensor([3], device=t0.device)
    t0[idx3] = k
    lst.append(t0)
    v4 = t1[:, 1:, 0:2:3]
    v4[3:4, 1:2, :] += 0
