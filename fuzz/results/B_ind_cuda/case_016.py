# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21000876
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = torch.cat([t1, t0], 0)
    v2 = torch.slice_scatter(t1, t0[0:1:3, :], dim=0, start=0, end=1, step=3)
    v3 = t2[-2, 1:2, 2]
    v2[:, :] = t0[:, :]
    v2[-3] = 3
    v4 = v2[0::2, :]
    ...
    v6 = v2.unsqueeze(0)
    t0.fill_(k)
    return (v3, v5, v2, t2, v3, lst, dct)

INPUTS = [('t0', (4, 6)), ('t1', (4, 6)), ('t2', (2, 3, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
