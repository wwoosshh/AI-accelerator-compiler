# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21001463
import torch

def fn(t0, t1, t2, ta, k, lst, dct):
    ta.fill_(2)
    v1 = t0[0:3:2, :]
    v2 = torch.diagonal_scatter(ta, v1.diagonal())
    v3 = v1.view_as(v1)
    v4 = torch.slice_scatter(t2, t0[1:2:2, :], dim=0, start=1, end=2, step=2)
    t2.zero_()
    v5 = v2[:]
    v4.add_(t2)
    return (v1, t2, v4, v4, v1, lst, dct)

INPUTS = [('t0', (3, 8)), ('t1', (6, 4)), ('t2', (3, 8))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
