# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21002674
import torch
...
def fn(t0, t1, ta, k, lst, dct):
    ...
    v2 = t1.unsqueeze(0)
    v3 = t1.t()
    idx4 = torch.tensor([0], device=t1.device)
    t1.index_fill_(1, idx4, 4)
    idx5 = torch.tensor([1, 0], device=t0.device)
    t0.index_fill_(1, idx5, -2)
    v6 = v1[2:3, :, 3::3]
    v7 = v6.squeeze(2)
    v8 = v2.clamp(0, 1)
    return (v6, v3, v3, t0, lst, dct)

INPUTS = [('t0', (6, 4)), ('t1', (3, 8))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cuda'
