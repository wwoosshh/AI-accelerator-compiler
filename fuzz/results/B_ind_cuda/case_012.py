# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21000600
import torch

def fn(t0, t1, k, lst, dct):
    v1 = torch.slice_scatter(t1, t0[4:9:1], dim=0, start=4, end=9, step=1)
    v2 = t0 + v1
    dct['key3'] = v1
    t1.zero_()
    v4 = torch.minimum(t1, v2)
    idx5 = torch.tensor([2, 15, 9], device=v4.device)
    v4[idx5] += 5
    return (t0, v2, t0, lst, dct)
