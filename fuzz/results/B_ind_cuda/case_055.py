# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21002467
import torch
...
def fn(t0, t1, k, lst, dct):
    ...
    v2 = v1.unfold(2, 2, 3)
    lst.append(t1)
    v3 = torch.ops.aten.alias(v2)
    v4 = v1.unsqueeze(0)
    t0[1:, :, :] = t0[:-1, :, :].clone()
    v5 = torch.select_scatter(v3, v2.select(1, 1), 1, 1)
    v6 = v3.contiguous()
    v3[:, :, :1, 1:2:3] = v5[:, :, :1, 1:2:3]
    v7 = v4[...]
    return (v5, t0, v7, v1, t1, lst, dct)

INPUTS = [('t0', (2, 2, 6)), ('t1', (4, 3, 2))]
ALIASED = None
DTYPE = 'f64'
DYNAMIC = None
DEVICE = 'cuda'
