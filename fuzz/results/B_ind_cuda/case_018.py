# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21000942
import torch

def fn(t0, t1, k, lst, dct):
    v1 = torch.ops.aten.alias(t1)
    v2 = torch.diagonal_scatter(t1, v1.diagonal())
    v3 = t0.unsqueeze(1)
    t1.fill_(2)
    v4 = v1.permute(0, 1)
    v5 = v1.view(torch.int32)
    v6 = torch.diagonal_scatter(v2, v1.diagonal())
    return (v1, v2, v1, t0, lst, dct)

INPUTS = [('t0', (3, 8)), ('t1', (6, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
