# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21000491
import torch

def fn(t0, t1, k, lst, dct):
    v1 = t0.view_as(t0)
    v2 = torch.select_scatter(t0, v1.select(0, 10), 0, 10)
    v1.copy_(v2)
    idx3 = torch.tensor([20, 19], device=t0.device)
    t0.scatter_add_(0, (v2.abs() % 24).long(), v2)
    return (t1, t1, v2, t0, t1, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (2, 12))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
