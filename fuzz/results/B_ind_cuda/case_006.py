# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21000346
import torch
...
def fn(t0, t1, k, lst, dct):
    ...
    v1 = t0.reshape((4, 6))
    v2 = t0.permute(0, 1, 2)
    idx3 = torch.tensor([0, 3, 2], device=v1.device)
    v1[idx3] += k
    idx4 = torch.tensor([0, 1], device=t1.device)
    t1.index_fill_(1, idx4, k)
    t1[t1 > -2] = k
    v2[t1 > -1] = k
    t0.transpose_(1, 2)
    return (v2, v2, v1, t0, v2, lst, dct)
