# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21001626
import torch
...
def fn(t0, t1, ta, k, lst, dct):
    ...
    v2[1:, :] = v2[:-1, :].clone()
    ta.mul_(0)
    idx3 = torch.tensor([0], device=v2.device)
    v2.index_fill_(1, idx3, -5)
    v4 = t0.unfold(0, 2, 1)
    ta[2] = 4
    idx5 = torch.tensor([2, 0, 1], device=v1.device)
    v1[idx5] += 5
    return (t0, v4, t1, t0, lst, dct)

INPUTS = [('t0', (2, 12)), ('t1', (3, 8))]
