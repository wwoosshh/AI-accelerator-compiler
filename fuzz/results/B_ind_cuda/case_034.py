# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21001651
import torch

def fn(t0, t1, k, lst, dct):
    idx1 = torch.tensor([16, 12], device=t0.device)
    t0.index_fill_(0, idx1, 3)
    dct['n'] = dct.get('n', 0) + 1
    v2 = t1.unsqueeze(1)
    v3 = t0[17:22]
    v4 = t0.unfold(0, 21, 24)
    v2.neg_()
    idx5 = torch.tensor([8, 2], device=v4.device)
    v4.index_fill_(1, idx5, k)
    return (v2, v2, v2, v2, t1, v2, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (2, 3, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
