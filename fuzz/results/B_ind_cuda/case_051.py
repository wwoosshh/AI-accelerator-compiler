# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21002280
import torch

def fn(t0, t1, k, lst, dct):
    v1 = t0[:, 7::2]
    idx2 = torch.tensor([6, 3, 1], device=t1.device)
    t1.index_add_(1, idx2, t0.narrow(1, 0, 3))
    dct['key3'] = t0
    v4 = t0.view(torch.int32)
    v4[v4 > -1] = -4
    v5 = t0.view(torch.int32)
    v6 = v1.narrow(0, 0, 1)
    return (v1, t1, t1, lst, dct)
