# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21001817
import torch

def fn(t0, t1, k, lst, dct):
    idx1 = torch.tensor([1, 0], device=t1.device)
    t1.index_fill_(0, idx1, -1)
    v2 = t1.sum(0, keepdim=False)
    v3 = v2.unfold(0, 3, 12)
    idx4 = torch.tensor([0], device=v3.device)
    v3[idx4] = 3
    idx5 = torch.tensor([0], device=v3.device)
    v3.index_fill_(0, idx5, -2)
    lst.append(t1)
    return (t1, t1, v3, v2, t0, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (2, 12))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
