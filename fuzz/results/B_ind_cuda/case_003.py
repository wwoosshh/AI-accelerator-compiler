# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21000220
import torch
...
def fn(t0, t1, k, lst, dct):
    ...
    v4 = t1[:, -5]
    t0[:-1, :, :] = t0[1:, :, :].clone()
    idx5 = torch.tensor([3, 1, 2], device=v4.device)
    v4.index_fill_(0, idx5, -2)
    idx6 = torch.tensor([0], device=v3.device)
    v3.index_fill_(2, idx6, 4)
    return (v1, t1, t0, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (4, 6))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
