# -*- coding: utf-8 -*-
# aliasfuzz repro | status=alias_diff backend=inductor device=cuda seed=21002125
import torch
...
def fn(t0, t1, t2, ta, k, lst, dct):
    ...
    t0.fill_(2)
    v1 = ta[1:8]
    idx2 = torch.tensor([12], device=t0.device)
    t0.index_fill_(0, idx2, 1)
    return (v1, t0, lst, dct)

INPUTS = [('t0', (24,)), ('t1', (4, 3, 2)), ('t2', (6, 4))]
ALIASED = ('ta', 't0[1:]')
DTYPE = 'i64'
DYNAMIC = True
DEVICE = 'cuda'
