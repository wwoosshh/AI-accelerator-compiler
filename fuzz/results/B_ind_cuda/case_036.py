# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21001765
import torch

def fn(t0, t1, t2, k, lst, dct):
    v1 = torch.select_scatter(t2, t1.select(0, 1), 0, 1)
    v2 = t1.unfold(1, 2, 2)
    idx3 = torch.tensor([1], device=v2.device)
    v2[idx3] += k
    return (t2, v1, v1, lst, dct)

INPUTS = [('t0', (2, 3, 4)), ('t1', (2, 3, 4)), ('t2', (2, 3, 4))]
ALIASED = None
DTYPE = 'i64'
DYNAMIC = None
DEVICE = 'cuda'
