# -*- coding: utf-8 -*-
# aliasfuzz repro | status=value_diff backend=inductor device=cuda seed=21001127
import torch

def fn(t0, t1, k, lst, dct):
    idx1 = torch.tensor([1, 6, 2], device=t0.device)
    t0.index_fill_(1, idx1, 4)
    v2 = t0.clone()
    lst.append(t1)
    t0.clamp_(-4, 4)
    t1.unsqueeze_(3)
    v3 = t1[:, 2:, :, 0]
    return (v2, v3, v2, v3, lst, dct)
