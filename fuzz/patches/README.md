...
PyTorch 클론(`C:\Users\s0105\src\pytorch`, 브랜치 `fix-reinplace-input-overwritten-later`)에 5개의 로컬 커밋으로 기록했고, 같은 내용을 여기 `.patch` 파일로 내보냈습니다. 아무 것도 pytorch/pytorch 에 푸시하지 않았습니다. 나이틀리 venv(`pt2nightly_cu`)의 site-packages 에는 0001·0002·0004·0005 가 적용된 상태이며 원본 백업은 `*.orig_nightly` 입니다.
...
설치된 휠에 직접 적용해 확인하려면 `reinplace.py`, `input_output_analysis.py`, `_dynamo/variables/tensor.py`, `_inductor/graph.py` 를 site-packages 의 같은 경로에 덮어쓰면 된다. Windows 에서 PyTorch 자체 테스트 러너(`python test_x.py`)는 출력이 없으니 `python -m pytest` 를 쓴다.
